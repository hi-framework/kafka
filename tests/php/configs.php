...
assert($connMap['bootstrap.servers'] === '127.0.0.1:9094');
assert($connMap['security.protocol'] === 'PLAINTEXT');
...
assert($prodMap['compression.type'] === 'lz4');
assert($prodMap['acks'] === 'all');
assert($prodMap['enable.idempotence'] === 'true');
...
assert($consMap['auto.offset.reset'] === 'earliest');
assert($consMap['isolation.level'] === 'read_committed');
