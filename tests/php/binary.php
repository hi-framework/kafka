...
echo "  value bytes (hex, head) : " . bin2hex(substr($value, 0, 20)) . "... (len=" . strlen($value) . ")" . PHP_EOL;
foreach ($headerValues as $i => $hv) {
    echo "  header[$headerNames[$i]]: " . bin2hex(substr($hv, 0, 16))
        . (strlen($hv) > 16 ? '...' : '') . " (len=" . strlen($hv) . ")" . PHP_EOL;
}
...
echo "  poll offset={$got['offset']} key_len=" . strlen($got['key'])
    . " value_len=" . strlen($got['value']) . " headers=" . count($got['headers']) . PHP_EOL . PHP_EOL;
...
echo "  ✓ key 完全一致 (" . strlen($key) . " bytes, 含 NUL)" . PHP_EOL;

if ($got['value'] !== $value) {
    fwrite(STDERR, "FAIL: value 字节不匹配\n");
    fwrite(STDERR, "  expected hex (head): " . bin2hex(substr($value, 0, 32)) . "\n");
    fwrite(STDERR, "  got hex      (head): " . bin2hex(substr($got['value'], 0, 32)) . "\n");
    fwrite(STDERR, "  expected len = " . strlen($value) . ", got len = " . strlen($got['value']) . "\n");
    exit(1);
}
echo "  ✓ value 完全一致 (" . strlen($value) . " bytes, 含 NUL + 0xFF + 非 UTF-8)" . PHP_EOL;
...
foreach ($headerNames as $i => $name) {
    ...
    echo "  ✓ header[$name] 完全一致 (" . strlen($expected) . " bytes)" . PHP_EOL;
}
