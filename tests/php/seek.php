...
echo '  消费前 5 条：' . \implode(', ', \array_map(static fn ($m) => "o={$m['offset']}/{$m['value']}", $consumed)) . \PHP_EOL;
...
echo '  seek 后重读：' . \implode(', ', \array_map(static fn ($m) => "o={$m['offset']}/{$m['value']}", $replayed)) . \PHP_EOL;

if (empty($replayed)) {
    \fwrite(\STDERR, "FAIL: seek 后没拿到消息\n");
    exit(1);
}
if (3 !== $replayed[0]['offset']) {
...
echo '  timestamp seek 后：' . \implode(', ', \array_map(static fn ($m) => "o={$m['offset']}/{$m['value']}", $lateOnly)) . \PHP_EOL;
