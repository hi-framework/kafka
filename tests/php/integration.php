...
// ============================================================================
step('1. ensure_worker 显式拉起', function () use ($client) {
    $client->ensureWorker();
});

// ============================================================================
step('1b. registerCluster default', function () use ($client) {
...
// ============================================================================
step('2. produce fire-and-forget × 10', function () use ($client, $topic) {
    for ($i = 0; $i < 10; $i++) {
...
// ============================================================================
step('3. produce sync × 10（验证 offset）', function () use ($client, $topic, $withKafka) {
    $lastOffset = -2;
    for ($i = 0; $i < 10; $i++) {
...
// ============================================================================
$subscriptionId = null;
step('4. subscribe', function () use ($client, $topic, $group, &$subscriptionId) {
...
if ($withKafka) {
    $consumedFnf = 0;
    $consumedSync = 0;
    step('5. poll 全部 20 条', function () use ($client, &$subscriptionId, &$consumedFnf, &$consumedSync) {
        $deadline = microtime(true) + 15;
        while (microtime(true) < $deadline) {
            $batch = $client->poll($subscriptionId, 100, 1500);
            foreach ($batch as $m) {
                if (str_starts_with($m['value'], 'fnf-')) $consumedFnf++;
                if (str_starts_with($m['value'], 'sync-')) $consumedSync++;
            }
            if ($consumedFnf + $consumedSync >= 20) break;
        }
        assertTrue($consumedFnf === 10, "fnf 收到 $consumedFnf/10");
        assertTrue($consumedSync === 10, "sync 收到 $consumedSync/10");
    });

    step('6. commit + unsubscribe', function () use ($client, &$subscriptionId) {
        $client->commit($subscriptionId);
        $client->unsubscribe($subscriptionId);
    });
} else {
    step('5. poll (LoggingConsumer dry-run)', function () use ($client, &$subscriptionId) {
    ...
    step('6. commit + unsubscribe（dry-run）', function () use ($client, &$subscriptionId) {
        $client->commit($subscriptionId);
        $client->unsubscribe($subscriptionId);
    });
}
...
step('7. 统计快照', function () {
    ...
    assertTrue($retry['failures'] === 0, "retry.failures != 0");
    assertTrue($resub['failures'] === 0, "resub.failures != 0");
