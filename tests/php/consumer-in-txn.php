...
echo '  poll: ' . \implode(', ', \array_map(static fn ($m) => "o={$m['offset']}/{$m['value']}", $batch)) . \PHP_EOL;
...
foreach ($batch as $m) {
    $client->produceSync('txn', $topicOut, 'k', \mb_strtoupper($m['value']), [], null, null, 5000);
}
...
if (! empty($leftover)) {
    \fwrite(\STDERR, 'FAIL: 同 group 重订阅 IN 应该为空（offset 应已 commit），实际拿到 '
        . \count($leftover) . ' 条：' . \json_encode(\array_map(static fn ($m) => $m['offset'], $leftover)) . "\n");
    exit(1);
}
...
echo '  poll: ' . \implode(', ', \array_map(static fn ($m) => "o={$m['offset']}/{$m['value']}", $batchB)) . \PHP_EOL;
