...
echo "  前 3 条: " . implode(', ', array_map(fn($m) => "o={$m['offset']}/{$m['value']}", $batch)) . PHP_EOL;
...
if (! empty($paused)) {
    fwrite(STDERR, "FAIL: pause 期间不该有消息: "
        . implode(', ', array_map(fn($m) => "o={$m['offset']}/{$m['value']}", $paused)) . "\n");
    exit(1);
}
...
echo "  resume 后: " . implode(', ', array_map(fn($m) => "o={$m['offset']}/{$m['value']}", $after)) . PHP_EOL;
...
if (! empty($pausedAll)) {
    fwrite(STDERR, "FAIL: pause-all 期间不该有消息: "
        . implode(', ', array_map(fn($m) => "o={$m['offset']}", $pausedAll)) . "\n");
    exit(1);
}
...
echo "  resume-all 后: " . implode(', ', array_map(fn($m) => "o={$m['offset']}/{$m['value']}", $tail)) . PHP_EOL;
