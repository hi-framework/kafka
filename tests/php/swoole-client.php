...
Coroutine\run(function () use ($socket, $brokers, $topic, $group, $N_PRODUCERS, $N_WORKERS, &$result) {
    ...
    // 跑一轮并发 produce 的小闭包
    $runConcurrentProduces = function (string $tag, int $offset) use ($client, $topic, $N_PRODUCERS): array {
        $wg = new WaitGroup();
        ...
        for ($i = 0; $i < $N_PRODUCERS; $i++) {
            ...
            $key = "$tag-k" . ($offset + $i);
            $val = "$tag-v" . ($offset + $i);
            Coroutine::create(function () use ($client, $topic, $key, $val, $wg, $sent, $failed) {
                try {
                    $r = $client->produceSync('default', $topic, $key, $val, 5000);
                    if ($r['ok']) $sent->add(1); else $failed->add(1);
                } catch (\Throwable $e) {
                    $failed->add(1);
    ...
    for ($w = 0; $w < $N_WORKERS; $w++) {
        $wg->add();
        Coroutine::create(function () use ($client, $sub, $wg, $consumed, $seen, $stopMs, $w, $totalProduced) {
            try {
    ...
    foreach (['r1' => 0, 'r2' => $N_PRODUCERS] as $tag => $offset) {
        for ($i = 0; $i < $N_PRODUCERS; $i++) {
            ...
            $row = $seen->get($key);
            if ($row === false || $row['val'] !== $expectedVal) {
